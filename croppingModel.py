...
from roi_align_cpp.modules.roi_align import RoIAlignAvg
from rod_align_cpp.modules.rod_align import RoDAlignAvg
...
class mobilenetv2_base(nn.Module):
    ...
    def __init__(self, loadweights=True, downsample=4,
                 model_path='pretrained_model/mobilenet_0.682_0.643_0.613_0.585_0.844_0.827_0.807_0.787_0.849_0.874.pth'):
        super(mobilenetv2_base, self).__init__()

        model = MobileNetV2(width_mult=1.0)

        if loadweights:
            mobilenets = torch.load(model_path)
            model.load_state_dict(mobilenets)
